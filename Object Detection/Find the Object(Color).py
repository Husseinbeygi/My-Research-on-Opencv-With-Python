...
def find_strawberry(image):
    ...
    min_red = np.array([56, 93, 56])
    max_red = np.array([0, 20, 13])
    #layer
    mask1 = cv2.inRange(image_blur_hsv, min_red, max_red)

    #birghtness of a color is hue
    # 170-180 hue
    min_red2 = np.array([170, 93, 56])
    max_red2 = np.array([180, 20, 13])
    ...
    circled = circle_contour(image, big_strawberry_contour)
    show(circled)
